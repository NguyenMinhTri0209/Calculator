...
class MainActivity : AppCompatActivity() {
    ...
    fun equalAction(view: View) {
        resultsTV.text = caculateResults()
    }

    private fun caculateResults(): String {
        val digitsOperator = digitsOperator()
        if (digitsOperator.isEmpty()) return "0"

        val timesDivision = timeDivisionCaculate(digitsOperator)
        if (timesDivision.isEmpty()) return "0"

        val result = addSubtractCalculate(timesDivision)
        return result.toString() //hay
    }
